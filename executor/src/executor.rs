...
/// Core runtime-creation transition, shared by the in-process
/// [`InMemExecutorTransport`](crate::InMemExecutorTransport). Spawns the
/// runtime (via the provider) and records it Running, or marks it Failed.
pub(crate) async fn create_core(
    ...
    registry.begin_create(id).await?;
