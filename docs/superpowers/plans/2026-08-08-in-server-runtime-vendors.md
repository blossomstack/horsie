...
- **The velos port and `crates/velos-runtime` deletion** — status: done. `VelosRuntimeVendor` is a structural twin of the Fly one, and the two now share `runtime_command` — they built byte-identical `sh -c` lines. It takes the default no-op `sweep_orphans`, because the velos API it uses has no listing; a leftover container is reclaimed by name on the next create instead. The crate, its Docker target, and its CI build/push/merge steps are gone.
