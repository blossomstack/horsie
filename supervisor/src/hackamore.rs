//! Mint-at-spawn integration with hackamore, the policy-governed credential
//! proxy. When a job carries a hackamore run-policy and the daemon is configured
//! against a hackamore deployment, the job spawn mints a fresh policy-bound token
//! from hackamore's admin API, materializes a synthetic per-job home directory,
...
//!
//! The hackamore *server location* (`admin_url` / `proxy_url`) is genuinely
//! deployment-global, so it lives in [`HackamoreMinter`]. The *policy* and its TTL
//! are a per-run resource — like the workflow and capability files — carried on
//! each job as a [`HackamoreRunPolicy`].
//!
//! Fail closed: a job that asked for a hackamore policy but cannot mint must abort
...
/// An opaque, policy-bound bearer token minted by hackamore for exactly one job
/// spawn. Newtype so a random string can never be passed where a minted token
/// is required.
#[derive(Debug)]
pub struct JobToken(String);

/// Wire mirror of hackamore's admin `POST /mint` response. hackamore owns this
...
/// Daemon-side hackamore handle: the deployment-global server location plus the
...
/// [`HackamoreMinter::mint`] from the job's [`HackamoreRunPolicy`].
#[derive(Clone)]
pub struct HackamoreMinter {
    admin_url: String,
    proxy_url: String,
    client: reqwest::Client,
}

impl HackamoreMinter {
    ...
    /// to hackamore's admin API with the policy's TTL, return the opaque token.
    /// Every error path is a spawn-aborting failure (fail closed).
    pub async fn mint(&self, policy: &HackamoreRunPolicy) -> Result<JobToken, String> {
        ...
        // hackamore's admin wire contract is camelCase: {"policy": ..., "ttlSeconds": ...}.
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| format!("hackamore mint: POST {url} failed: {e}"))?;
        let status = resp.status();
        if !status.is_success() {
            let detail = resp.text().await.unwrap_or_default();
            return Err(format!(
                "hackamore mint: {url} returned HTTP {status}: {}",
                detail.trim()
            ));
        }
        let minted: MintResponse = resp
            .json()
            .await
            .map_err(|e| format!("hackamore mint: invalid response from {url}: {e}"))?;
...
pub struct HackamoreProvision {
    /// `HACKAMORE_TOKEN` / `HACKAMORE_URL` / `HOME` / `GH_CONFIG_DIR` for the child.
    pub env: Vec<EnvVar>,
...
pub async fn provision_for_spawn(
    minter: &HackamoreMinter,
    policy: &HackamoreRunPolicy,
    job_dir: &Path,
) -> Result<HackamoreProvision, String> {
...
/// spawn-side wiring is testable without a hackamore.
fn materialize_home(
    job_dir: &Path,
    proxy_url: &str,
    token: JobToken,
    ...
    std::fs::create_dir_all(&gh_config_dir)
        .map_err(|e| format!("hackamore: cannot create job home {}: {e}", home.display()))?;
    Ok(HackamoreProvision {
        env: vec![
            EnvVar {
                name: ENV_HACKAMORE_TOKEN.to_string(),
                value: token.0,
            },
            EnvVar {
                name: ENV_HACKAMORE_URL.to_string(),
...
/// hackamore variables. `None` (no minter configured, or a job that carries no
/// hackamore policy) passes both through untouched — byte-for-byte identical to a
/// daemon without hackamore.
pub fn apply_provision(
    caps: CapabilitySpec,
    provision: Option<HackamoreProvision>,
...
mod tests {
    ...
    /// Spin a mock hackamore admin API on an ephemeral port. Returns its base URL
    ...
    fn policy(ttl: u64) -> HackamoreRunPolicy {
        HackamoreRunPolicy {
            policy: json!({ "targets": [{ "name": "github" }] }),
            params: Some(HackamorePolicyParams {
                ttl_seconds: Some(ttl),
            }),
        }
    }

    /// Parse a per-run hackamore policy file (the CLI's `--hackamore-policy` path) into
    /// the generated wire type, mirroring how the daemon deserializes it.
    fn parse_policy(text: &str) -> Result<HackamoreRunPolicy, serde_json::Error> {
        serde_json::from_str(text)
    }

    fn minter(admin_url: &str) -> HackamoreMinter {
        HackamoreMinter::new(admin_url.to_string(), "http://127.0.0.1:9090".to_string())
    }
    ...
    async fn mint_posts_camelcase_body_and_returns_token() {
        ...
        let body = captured.lock().unwrap().clone().expect("body captured");
        // hackamore's wire contract: the policy doc passes through verbatim and the
    ...
    async fn provision_for_spawn_materializes_home_env_and_grant() {
        ...
        assert_eq!(lookup("HACKAMORE_TOKEN"), "tok-1");
        assert_eq!(lookup("HACKAMORE_URL"), "http://127.0.0.1:9090");
    ...
    fn apply_provision_absent_changes_nothing() {
        // A job with no hackamore policy (or no minter): capabilities pass through
        // untouched and no env is injected — identical to behavior before hackamore.
