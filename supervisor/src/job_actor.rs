...
impl JobRuntime for ProcessJobRuntime {
    async fn launch(&self, params: LaunchParams) -> Result<LaunchedJob, String> {
        ...
        // Mint-at-spawn (fail closed): when the daemon is configured against hackamore
        ...
        // spec, and inject the env the runtime needs to self-provision. No hackamore
        // config, or a job with no `--hackamore-policy`, → both stay untouched.
        let provision = match (&self.deps.hackamore, &spec.hackamore_policy) {
            (Some(minter), Some(policy)) => {
                Some(crate::hackamore::provision_for_spawn(minter, policy, &jdir).await?)
            }
            _ => None,
        };
        let (capabilities, env) =
            crate::hackamore::apply_provision(spec.capabilities.clone(), provision);
