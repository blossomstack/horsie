...
/// Result of a turn, interpreted by [`conclusion::conclude`].
...
async fn finish(
    ...
    let tail = conclude(report, folded, cx).await;
...
fn spawn_call(
    ...
    tokio::spawn(async move {
        ...
        let cmd = match horsie_agentcore::run_step(&req, &history, &sink, &cancel).await {
            ...
            Err(StepError::Provider(error)) => ProviderCommand::StepFailed { marker_seq, error },
        };
...
async fn handle_responded(
    ...
    if tool_calls.is_empty() {
        events.push(turn_completed(&folded));
        let folded = RunLoop::apply_all(cx.state, &events);
        return finish(
            events,
            RunReport {
...
async fn handle_llm_failed(
    ...
    if error.is_transient() && attempt < max_retries {
        ...
        let state = cx.state.clone();
        spawn_call(&state, cx, next_attempt, Some(delay));
...
pub(crate) async fn ended_by_tools(
    ...
    events.push(turn_completed(cx.state));
    let folded = RunLoop::apply_all(cx.state, &events);
    finish(
        events,
        RunReport {
...
pub(crate) async fn handle(
    ...
    match cmd {
        ProviderCommand::StepDone {
            ...
            handle_responded(*response, cx).await
        }
        ProviderCommand::StepFailed { marker_seq, error } => {
            ...
            handle_llm_failed(error, attempt, cx).await
        }
        ProviderCommand::StreamDelta { marker_seq, text } => {
            if open_agent_marker(cx.state, marker_seq) && cx.step_run.push_delta(marker_seq, text) {
                cx.publish_revision();
            }
            CommandEffect::none()
        }
