//! Making a history well-formed again after a tool call was left unanswered.
//!
//! A provider rejects a request in which any `tool_use` has no matching
//! `tool_result`, and it rejects the whole request — so one call interrupted by
//! a Stop or a crash bricks every later turn of that session, not just the turn
//! it happened in.
//!
//! Two halves, deliberately not the same function. One produces the synthetic
//! results to *journal* at the moment a call becomes permanently unanswerable;
//! the other patches the history put *on the wire* and journals nothing. The
//! second is the guard that makes the first safe to skip for a parked call,
//! which is waiting for an answer rather than wreckage.
