...
impl Handler for AskUserCapability {
    ...
    fn handle(&self, caller: Caller, msg: &Message) -> Option<Decision> {
        ...
        match msg {
            Message::Tool(t) if t.name == TOOL => Some((
                vec![CapEvent::AskUser(Event::Asked {
                    agent: caller.agent,
                })],
                vec![],
            )),
            ...
            Message::Ask(AskMsg::Answered { .. }) if self.pending.is_some() => {
                Some((vec![CapEvent::AskUser(Event::Answered)], vec![]))
            }
...
mod tests {
    ...
    #[test]
    fn unattended_equips_nothing_and_declines_the_call() {
    ...
    #[test]
    fn an_attended_session_equips_the_layer() {
        let mut spec = AgentSpec::default();
        AskUserCapability::new().setup(&mut spec);
        assert!(spec.has(&ToolLayer::AskUser));
    }
}
