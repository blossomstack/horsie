...
/**
 * Flatten a session's sub sessions into render order: each sub session
 * immediately after the one it branched from, carrying how deep it sits.
 *
...
export function subSessionTree(subSessions: SubSessionView[]): PlacedSubSession[] {
  ...
  const walk = (parent: string, depth: number) => {
    for (const subSession of byParent.get(parent) ?? []) {
      // The descent can reach a node twice if the data is not a tree. This
      // walks a journal, and placing a row twice is worse than misplacing it.
      if (seen.has(subSession.id)) continue;
      seen.add(subSession.id);
      out.push({ subSession, depth });
      walk(subSession.id, depth + 1);
    }
  };
  walk("", 0);
...
export function subSessionReadyToOpen(
  ...
  const row = subSessions?.find((row) => row.id === id);
