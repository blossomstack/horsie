...
/// Wall-clock budget for one `` !`cmd` `` in a slash command's template.
///
/// Short on purpose: this runs while the user waits for their turn to start,
/// and every published use of it is a `git status`-shaped one-liner.
const COMMAND_SHELL_TIMEOUT_SECS: u64 = 15;
...
impl SessionContextProvider {
    ...
    /// Expand a slash command, if this prompt is one.
    ///
    /// Returns the expanded message and whatever `UserPromptExpansion` hooks
    /// produced. `None` leaves the prompt exactly as the user wrote it — which
    /// covers a message that merely begins with a slash, since an unknown name
    /// is not an error.
    ///
    /// **The library is scanned only when the prompt starts with `/`.** This
    /// seam runs before `provide()` and so has no scan of its own, and paying
    /// for one on every turn to serve a feature used on a few of them is the
    /// wrong trade.
    async fn expand_command(
        ...
        use horsie_support::plugin::commands as sc;

        let Some((name, args)) = sc::parse_invocation(prompt) else {
            return (None, Vec::new());
        };
        if !self.use_plugins() {
            return (None, Vec::new());
        }
        let (_, scan) = scan_workspace(client, None, true).await;
        let Some(command) = scan.commands.get(name) else {
            // Not a command horsie knows. Left alone rather than refused: a
            // message may legitimately begin with a slash.
            return (None, Vec::new());
        };
        ...
        let records = client
            ...
            .await
            .unwrap_or_default();
        // A hook that refused stops the expansion; the refusal is read off the
        // records by `start_blocked`, exactly as a `UserPromptSubmit` block is.
        if halt_reason(&records).is_some() {
            return (None, records);
        }

        // `!`cmd`` runs only for a command that declared Bash. A template
        // interpolating a shell without asking for one is a template whose
        // author expected the declaration to matter.
        let may_shell = command
            .def
            .allowed_tools
            .iter()
            .any(|t| horsie_support::plugin::hooks::horsie_tools_for(t).contains(&"bash"));
        let mut shell = std::collections::BTreeMap::new();
        if may_shell {
            for snippet in sc::shell_snippets(&command.def.template) {
                // A failure substitutes its own error text: the model can see
                // what went wrong, and a broken snippet does not sink the turn.
                // Through the ordinary tool path, so the runtime's own
                // `PreToolUse` guards apply: a hook that refuses this shell
                // refuses it here too, and says so in the substituted text.
                let call =
                    horsie_models::runtime::ToolCall::Bash(horsie_models::runtime::BashInput {
                        command: snippet.clone(),
                        timeout_secs: Some(COMMAND_SHELL_TIMEOUT_SECS),
                    });
                let output = match client.invoke(&format!("cmd-expand:{name}"), call).await {
                    Ok(out) => out.stdout.trim_end().to_string(),
                    Err(e) => format!("[command shell failed: {e}]"),
                };
                shell.insert(snippet, output);
            }
        }
        let body = sc::expand(&command.def.template, args, &shell);
        (Some(sc::frame(name, args, &body)), records)
    }
...
impl ContextProvider for SessionContextProvider {
    ...
    async fn start_hooks(&self, turn: StartTurn) -> Result<TurnPreparation, ContextError> {
        ...
        if let Some(prompt) = turn.prompt {
            ...
            // puts it: a guard reads the prompt the model will actually see.
            let (expansion, expansion_records) = self.expand_command(&client, &prompt).await;
    ...
    async fn provide(&self) -> Result<Contexts, ContextError> {
        ...
        *self
        ...
        let shared = use_plugins.then(|| SharedContext {
            ...
            commands: Arc::new(shared_scan.commands),
...
mod tests {
    ...
    /// A session whose library declares `/review`, with a runtime that answers
    /// the real scan.
    async fn command_harness(
        commands: Vec<horsie_models::runtime::PluginCommand>,
    ...
    fn command_file(name: &str, front: &str, body: &str) -> horsie_models::runtime::PluginCommand {
        horsie_models::runtime::PluginCommand {
            plugin: "fd".into(),
            rel_path: format!("fd/commands/{name}.md"),
            content: format!("---\n{front}---\n{body}"),
        }
    }
    ...
    async fn a_slash_command_expands_into_its_framed_template() {
        let (f, session, id) = command_harness(vec![command_file(
            "review",
            "description: reviews a file\n",
            "Review $1 for bugs. Full args: $ARGUMENTS",
        )])
        .await;
        let provider = provider_for_session(&f, &session, id);
    ...
    #[tokio::test]
    async fn an_unknown_command_leaves_the_prompt_alone() {
        let (f, session, id) = command_harness(vec![command_file(
            "review",
            "description: reviews\n",
            "body",
        ...
        .await;
        let provider = provider_for_session(&f, &session, id);
        for prompt in ["/nosuch thing", "/etc/hosts is a file", "hello"] {
            assert_eq!(
    ...
    /// `` !`cmd` `` runs only for a command that declared Bash. A template
    /// interpolating a shell without asking for one gets nothing.
    #[tokio::test]
    async fn a_shell_snippet_needs_the_command_to_declare_bash() {
        let (f, session, id) = command_harness(vec![
            command_file(
                "with",
        ...
        .await;
        let provider = provider_for_session(&f, &session, id);
        // The fake runtime answers every bash call with its scripted stdout.
        let with = prepared_message(&provider, "/with").await.unwrap();
        assert!(
            with.contains("status: ok"),
            "declared Bash, so it ran: {with}"
        );
        let without = prepared_message(&provider, "/without").await.unwrap();
        assert!(
            without.contains("status: \n") || without.contains("status: <"),
            "no Bash declared, so the snippet leaves nothing: {without:?}"
        );
    }

    /// `UserPromptExpansion` fires for the command being expanded, carrying its
    /// name as the matcher domain — and before `UserPromptSubmit` sees the
    /// result, which is the order the spec gives them.
    #[tokio::test]
    async fn expansion_is_hooked_before_submission() {
        ...
        let commands: Vec<String> = f
            ...
            .into_iter()
            .filter_map(|e| match e {
                horsie_models::runtime::ServerHookEvent::UserPromptExpansion(i) => Some(i.command),
                _ => None,
            })
            .collect();
    ...
    /// A prompt that cannot be a command never scans the library, which is what
    /// keeps expansion free on the turns that do not use it.
    #[tokio::test]
    async fn an_ordinary_prompt_fires_no_expansion_hook() {
        let (f, session, id) = command_harness(vec![command_file(
            "review",
            "description: reviews\n",
            "body",
        ...
        .await;
        let provider = provider_for_session(&f, &session, id);
        prepared_message(&provider, "just a message").await;
        assert!(
            !f.agent.hook_events().contains(&"UserPromptExpansion"),
            "{:?}",
            f.agent.hook_events()
        );
    }
